...
function CVDM_BF(id,nb) {
	...
	$.post("{/literal}{$baseurl}/votec_no_date.php{literal}",{"id":id,"nb":nb},function(html) {
			$('#CDvotenobdate'+id).html(html).fadeIn(); 
		});
}
										...
										<div class="right">
											<div class="original_text">
												...
												<div class="text_language">
													<div class="edit_left"><span class="silver_nn">
													{if $parray.city}<p>{$parray.city}, {$parray.region}, {$parray.country}</p>{/if}
													<p>{$lang162}: {if $parent ne "0"}<a href='{$baseurl}/{$pseo}'>{$pname|stripslashes}</a> - {/if}<a href='{$baseurl}/{$nseo}'>{$name|stripslashes}</a><br/></p>
													<p style="word-wrap: break-word;">{$lang184}: {insert name=get_tags assign=looptags tags=$parray.tags}{section name=tt loop=$looptags}<a href="{$baseurl}/search?query={$looptags[tt]|stripslashes}">{$looptags[tt]|stripslashes}</a>&nbsp;{/section}<br/></p></span>
													</div>
													<div class="clear">	</div>
												</div>
												<div class="share">
											...
											<div style="margin-bottom:10px;padding:5px 0px 5px 0px"><span class="silver_nn">
												<ul class="sortby">
												...
												<div style="float: none; width: auto; word-wrap: break-word;background: #eee;padding: 10px;">
													<div class="edit_text">
														<div class="edit_left">
															...
															{/if}
															{if $showcomments2[vc].address && $showcomments2[vc].city}<iframe width="100%" height="200" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.ca/maps?f=q&source=s_q&hl=en&geocode=&q={$showcomments2[vc].address|replace:' ':'+'}+{$showcomments2[vc].city|replace:' ':'+'}+{$showcomments2[vc].region|replace:' ':'+'}+{$showcomments2[vc].country|replace:' ':'+'}&z=14&output=embed"></iframe>{/if}
															</div>
														</div>
														<div class="edit_right">
															<div class="thumbs-rating">
																<div class="ans_rate">
																	<img src="{$imageurl}/icon_goodno.gif"/ title="Yes, I did">
																		<span id='CDvote{$showcomments2[vc].CID}'>
																		<a class='positive' href="javascript:;" onclick="CVDM('{$showcomments2[vc].CID}','{$showcomments2[vc].vote_yes}');" >
																		<span id='Cvote{$showcomments2[vc].CID}'>{$showcomments2[vc].vote_yes}</span>
																		</a>
																		</span>
																		<a class='positive' id='CD2vote{$showcomments2[vc].CID}' href="#" style="display:none"></a>
																		<br/>
																		<span class="ans_rate_date" id='CDvoteyesdate{$showcomments2[vc].CID}'>{if $showcomments2[vc].vote_yes > 0}{$showcommentsw[vc].thumbedtime|date_format}{/if}</span>
																</div>
																<div class="ans_rate">
																	<img src="{$imageurl}/icon_badno.gif"/ title="No, I didn’t.">
																		<span id='CDvoteb{$showcomments2[vc].CID}'>
																			<a class='negative' href="javascript:;" onclick="CVDM_BF('{$showcomments2[vc].CID}','{$showcomments2[vc].vote_no}');"><span id='Cvotebf{$showcomments2[vc].CID}'>{$showcomments2[vc].vote_no}
																		</span></a></span><a class='negative' id='CD2voteb{$showcomments2[vc].CID}' href="#" style="display:none"></a>
																		<br/>
																		<span class="ans_rate_date" id='CDvotenodate{$showcomments2[vc].CID}'>{if $showcomments2[vc].vote_no > 0}{$showcomments2[vc].thumbedtime_no|date_format}{/if}</span>
																</div>
																<div class="clear">	</div>
															</div>
														</div>
											...
											<div class="correct_loop" id="answer_{$showcomments[vc].CID}">
												...
												<div class="question-answer">
													<div class="edit_text">
														<div class="edit_left">
															...
															{if $showcomments[vc].address && $showcomments[vc].city}
																<iframe width="100%" height="200" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.ca/maps?f=q&source=s_q&hl=en&geocode=&q={$showcomments[vc].address|replace:' ':'+'}+{$showcomments[vc].city|replace:' ':'+'}+{$showcomments[vc].region|replace:' ':'+'}+{$showcomments[vc].country|replace:' ':'+'}&z=14&output=embed"></iframe>
															{/if}
															</div>
														</div>
														<div class="edit_right">
															<div class="thumbs-rating">
																<div class="ans_rate">
																<img src="{$imageurl}/icon_goodno.png"/ title="Yes, I do."><span id='CDvote{$showcomments[vc].CID}'><a class='positive' href="javascript:;" onclick="CVDM('{$showcomments[vc].CID}','{$showcomments[vc].vote_yes}');"><span id='Cvote{$showcomments[vc].CID}'>{$showcomments[vc].vote_yes}</span></a></span><a class='positive' id='CD2vote{$showcomments[vc].CID}' href="#" style="display:none"></a>
																<br/>
																<span class="ans_rate_date" id='CDvoteyesbdate{$showcomments[vc].CID}'>{if $showcomments[vc].vote_yes > 0}{$showcomments[vc].thumbedtime|date_format}{/if}</span>
																</div>
																<div class="ans_rate">
																<img src="{$imageurl}/icon_badno.png"/ title="No, I didn't."><span id='CDvoteb{$showcomments[vc].CID}'><a class='negative' href="javascript:;" onclick="CVDM_BF('{$showcomments[vc].CID}','{$showcomments[vc].vote_no}');"><span id='Cvotebf{$showcomments[vc].CID}'>{$showcomments[vc].vote_no}</span></a></span><a class='negative' id='CD2voteb{$showcomments[vc].CID}' href="#" style="display:none"></a>
																<br/>
																<span class="ans_rate_date" id='CDvotenobdate{$showcomments[vc].CID}'>{if $showcomments[vc].vote_no > 0}{$showcomments[vc].thumbedtime_no|date_format}{/if}</span>
																</div>
																<div class="clear">	</div>
															</div>
														</div>
													...
													<div class="original_comment">
														...
														<div class="clear"></div>
														{if $smarty.session.USERID ne "" AND $smarty.session.USERID eq $parray.USERID AND $parray.open eq "1" AND $showcomments[vc].USERID ne $parray.USERID}<br/>
														<form method="post">															
															<input type="submit" name="selectbest" value="{$lang198}" />															
															<input type="hidden" name="CID" value="{$showcomments[vc].CID}" />
														</form>
														{/if}
														<div class="clear"></div>
													</div>
