...
function insert_get_post_picture($var){
        global $conn;
        $query="SELECT picture FROM posts WHERE PID='".mysql_real_escape_string($var['PID'])."' limit 1";
        $executequery=$conn->execute($query);
		$results = $executequery->fields['picture'];
		if ($results == "")
