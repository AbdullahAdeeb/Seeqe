...
{
	$query="UPDATE posts SET vote_yes=vote_yes+1,  thumbedtime=now() WHERE PID='".mysql_real_escape_string($PID)."'";
	$result=$conn->execute($query);
	$expire=time()+60*60*24;
	setcookie($cname, "yes", $expire);
	echo '<span class="thumbsUp" id="Dvote{$parray.PID}">'.$votes.'</span>';
}
else
{
	echo '<span class="thumbsUp" id="Dvote{$parray.PID}">'.$nb.'</span>';
}
?>